...
const User_reviewPage = observer(() => {
    ...
    const DataSet = [
        {
            columns: [
                {title: "№", style: {font: {sz: "18", bold: true}}, width: {wpx: 50}},
                {title: "Произведение", style: {font: {sz: "18", bold: true}}, width: {wch: 50}},
                {title: "Заголовок", style: {font: {sz: "18", bold: true}}, width: {wpx: 50}},
                {title: "Текст", style: {font: {sz: "18", bold: true}}, width: {wpx: 50}}

            ],
            data: exportData.map((data, key) => [
                {value: key+1, style: {font: {sz: "14"}}},
                {value: data.name_composition, style: {font: {sz: "14"}}},
                {value: data.title,  style: {font: {sz: "14"}}},
                {value: data.text,  style: {font: {sz: "14"}}}
            ])
        }
    ]
    return (
            ...
            <Row className="mt-2">
                ...
                <Col md={9}>
                    ...
                    <div className="d-flex justify-content-md-end align-items-center">
                        ...
                        {exportData.length !== 0 ? (
                            <ExcelFile
                                filename="Мои рецензии"
                                element={<button type="button" className="btn btn-success float-right m-3">Экпорт рецензий</button>}>
                                <ExcelSheet dataSet={DataSet} name="Мои рецензии"/>
                            </ExcelFile>
                        ): null}
